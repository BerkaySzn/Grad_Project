...
def find_recipes_by_ingredients(ingredients):
    ...
    try:
        ...
        # Remove duplicates
        unique_recipes = list({recipe.recipe_id: recipe for recipe in all_recipes}.values())
        print(f"Found {len(unique_recipes)} unique recipes")

        # Convert to dictionary format
        recipe_list = []
        for recipe in unique_recipes:
            recipe_details = get_recipe_with_details(recipe.recipe_id)
            if recipe_details:
                recipe_list.append(recipe_details)
                print(f"Added recipe: {recipe_details['name']}")
...
def favorites():
    ...
    recipes_with_details = [get_recipe_with_details(recipe.recipe_id) for recipe in favorite_recipes]
    return render_template("favorites.html", user=current_user, recipes=recipes_with_details)
